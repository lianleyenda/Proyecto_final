...
def iniciar_carrito():
    ...
    if "carrito" not in session:
        session["carrito"] = []
    if "carrito_promo" not in session:
...
def agregar_promo(id):
    ...
    carrito_promo = session["carrito_promo"]

    for p in carrito_promo:
        if p["id"] == item["id"]:
            p["cantidad"] += 1
            session.modified = True
            break
...
def admin_eliminar_producto(id_Precio):
    ...
    cursor.execute("DELETE FROM Productos WHERE id_Precio = %s", (id_Precio,))
    db.commit()
    filas_afectadas = cursor.rowcount
