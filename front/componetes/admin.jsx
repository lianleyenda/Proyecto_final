...
function Admin() {
  ...
  const agregarProducto = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMenu([...menu, { id_Stock: data.id, ...nuevoProducto }]);
        setNuevoProducto({
          Producto: "",
          Costo: "",
          Descripcion: "",
  ...
  const eliminarProducto = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(data.mensaje);
        setMenu(menu.filter((item) => item.id_Stock !== id));
      } else {
    ...
    <div>
      ...
      <div className="contenedor-admin">
        ...
        <form onSubmit={agregarProducto} className="form-admin">
          ...
          <input
            type="text"
            placeholder="Nombre de imagen (ej: burger1.png)"
          ...
          <ul>
            {menu.map((item) => (
              <ol key={item.id_Stock}>
                ...
                <button onClick={() => eliminarProducto(item.id_Stock)}>
                  🗑️ Eliminar
                </button>
              </ol>
            ))}
